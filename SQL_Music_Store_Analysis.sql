...
--Q4. query the city and highest sum of all invoice total 

select billing_city , sum(total) from invoice
group by billing_city
order by sum(total) desc limit 1;

--Q.5 query the best customer
select invoice.customer_id, first_name , last_name ,  sum(total)
...
select artist.name , count(track_id) as totalTrackCount
